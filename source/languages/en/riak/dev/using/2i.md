...
[[Riak Search|Using Search]] serves analogous purposes but is quite different because it parses key/value data itself and builds indexes on the basis of custom schemas.
...
* Query results can be used as input to a [[MapReduce|Using MapReduce]] query
...
* When you need more than the exact match and range searches that 2i supports. If that's the case, we recommend checking out [[Riak Search|Using Search]].
...
```java
Location johnSmithKey = new Location("users")
...
```curl
curl -XPOST localhost:8098/types/<type>/buckets/users/keys/john_smith \
...
```java
Location usersBucket = new Location("users");
...
```python
bucket = client.bucket('users') 
# The above is equivalent to client.bucket_type('default').bucket('users')
...
```java
Location peopleBucket = new Location("people").setBucketType("indexes");
...
* `Veronica` --- Binary index `field1_bin` with the values `val4`, `val4a`, and `val4b` and integer index `field2_int` with the values `1004` and `1007` (note that redundancies have been removed)
...
```java
/*
  The Java client will not allow you to provide invalid
  index names, because you are not required to add "_bin" or "_int" to
  the end of index names.
*/
```

```python
bucket = client.bucket_type('<type>').bucket('mybucket')
...
```curl
curl -XPUT 127.0.0.1:8098/types/<type>/buckets/mybucket/keys/mykey \
...
```java
Location key = new Location("mybucket")
        .setKey("mykey");
RiakObject obj = new RiakObject();
...
```ruby
bucket = client.bucket('mybucket')
bucket.get_index('field1_bin', 'val1', type: '<type>')
```

```java
Location myBucket = new Location("mybucket").setBucketType("<type>");
...
{ok, Results} = riakc_pb_socket:get_index(
    Pid,
    {<<"<type>">>, <<"mybucket">>}, %% bucket type and bucket name
    {binary_index, "field2"},
    <<"val1">>
).
```
...
```curl
curl localhost:8098/types/<type>/buckets/mybucket/index/field1_bin/val1
```
...
```ruby
bucket = client.bucket('mybucket')
bucket.get_index('field1_int', 1001, type: '<type>')
```
...
```curl
curl localhost:8098/types/<type>/buckets/mybucket/index/field2_int/1001
```

The following example performs an exact match query and pipes the results into a MapReduce job. We'll query the bucket 
...
```ruby
bucket = client.bucket('mybucket')
bucket.get_index('field1_bin', 'val2'..'val4', type: '<type>')
```
...
```curl
curl localhost:8098/types/<type>/buckets/mybucket/index/field1_bin/val2/val4
```
...
```ruby
bucket = client.bucket('mybucket')
bucket.get_index('field2_int', 1002..1004, type: '<type>')
```
...
```curl
curl localhost:8098/types/<type>/buckets/mybucket/index/field2_int/1002/1004
```
...
bucket.get_index('hashtags_bin', 'rock'..'rocl', return_terms: true, type: '<type>')
```

```java
Location tweetsBucket = new Location("tweets").setBucketType("<type>");
...
```python
bucket = client.bucket_type('<type>').bucket('tweets')
...
```curl
curl localhost:8098/types/<type>/buckets/tweets/index/hashtags_bin/rock/rocl?return_terms=true
```
...
```ruby
bucket = client.bucket('tweets')
bucket.get_index('hashtags_bin', 'ri'..'ru', max_results: 5, type: '<type>')
```
...
{ok, Results} = riakc_pb_socket:get_index_range(
    Pid,
    {<<"<type>">>, <<"tweets">>}, %% bucket type and bucket name
    {binary_index, "hashtags"}, %% index name
...
```curl
curl localhost:8098/types/<type>/buckets/tweets/index/hashtags_bin/ri/ru?max_results=5&return_terms=true
```
...
```curl
curl localhost:8098/types/<type>/buckets/tweets/index/hashtags_bin/ri/ru?continuation=g2gCbQAAAAdyaXBqYWtlbQAAABIzNDkyMjA2ODcwNTcxMjk0NzM=&max_results=5&return_terms=true
```
...
```ruby
bucket = client.bucket('mybucket')
bucket.get_index('myindex_bin', 'foo', stream: true, type: '<type>')
```
...
```curl
curl localhost:8098/types/<type>/buckets/mybucket/index/myindex_bin/foo?stream=true
```
...
```curl
curl localhost:8098/types/<type>/buckets/mybucket/index/\$bucket/_
```
